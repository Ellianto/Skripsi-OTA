...
statusfile_path = uftp_dir / 'status.txt'
logfile_path = uftp_dir / 'uftp_client_logfile.txt'
...
def uftp_client_runner():
    return_code = None

    try:
        directory_check()
        proc_name = 'uftpd.exe'
        uftpd_process = next(p for p in psutil.process_iter()
                            if p.name() == proc_name)

    except StopIteration:
        # via psutil Module
        uftp_client = psutil.Popen(
            uftp_client_cmd, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
